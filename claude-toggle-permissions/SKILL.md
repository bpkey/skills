...
2. **Terminal guard.** Because the takeover ends the current session and relaunches in this exact tab, it only supports Apple Terminal. On any other known terminal (`iTerm.app`, `WarpTerminal`, `ghostty`, …) it refuses up front rather than killing the session and opening a stray Terminal.app window. An empty `TERM_PROGRAM` is treated as Apple Terminal (best effort). It also refuses inside **tmux/screen** (whose pty no Terminal tab can address) — `screen` leaves `TERM_PROGRAM` as the host terminal, so it's caught by an explicit `TMUX`/`STY`/`TERM` check.
3. **Identifies the interactive `claude` that owns this tab** by walking up the process tree from `$PPID` to the nearest ancestor whose command is `claude` *and* that has a real `ttys*` tty. (`$PPID` itself is a transient, ttyless `zsh -c`/`bash -c` wrapper Claude Code wraps script-path invocations in — it is **not** the session, and trusting it was the original bug.) Requiring `claude` means it never targets the login shell. If no such ancestor exists, it exits **without ending any session**.
4. Finds the current session's transcript by globbing the session id (`$CLAUDE_CODE_SESSION_ID`) across all `~/.claude/projects/*/` buckets — the id is globally unique, so this is **encoding- and cwd-agnostic** (it sidesteps Claude Code's project-dir encoding, which maps not just `/` but also `.`, `_`, and spaces to `-`). Fallbacks (only if the env var is absent): `lsof` on the interactive claude, then the newest JSONL in the cwd bucket (best-effort, warns).
5. Reads the latest `permissionMode` from the transcript — the session's **current effective** mode, including one you shift-tabbed into, not the original launch flag. `bypassPermissions` → flip **off** (relaunch without the flag); anything else → flip **on** (append `--dangerously-skip-permissions`). If no mode can be read, it warns and assumes not-bypass.
6. Reads the current session's display name (if any) from `~/.claude/sessions/<pid>.json` and builds the `-bypass` / `-safe` name.
7. **Pre-flights Automation permission** (a foreground `osascript ... count windows`). If it can't drive Terminal, it prints how to grant the permission and exits *without ending the session*.
8. Spawns a **detached watcher** (`nohup`, ignores HUP/TERM/INT) and prints a one-line summary. The watcher then: gives this turn ~1.5s to render → re-verifies the target pid is still that `claude` (same command + start time, so a recycled pid is never signalled) → `kill -TERM` (escalating to `-KILL` after up to ~5s) so the tab's shell returns to its prompt → finds the tab whose `tty` matches this one → runs `cd <dir> && echo <breadcrumb> && claude --resume <id> --fork-session -n <name> [--dangerously-skip-permissions]` in that tab.
...
- The takeover **ends the current session** (`SIGTERM` at ~1.5s, escalating to `SIGKILL` after up to ~5s if it doesn't exit). It is not a graceful in-session exit — the flipped session normally reappears within ~2–3s, up to ~7s if the old session is slow to exit.
...
- Apple Terminal only — the skill refuses to run in other terminals, and inside tmux/screen, so it never kills your session in a terminal it can't take over.
- Resolves the host `claude` and the tab tty by walking up from `$PPID` to the nearest ancestor whose command contains `claude` and owns a real `ttys*` terminal — `$PPID` itself is the transient ttyless `zsh -c`/`bash -c` wrapper Claude Code wraps script-path invocations in, so it is deliberately not used directly. If no qualifying ancestor is found, it exits without ending any session.
