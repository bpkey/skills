...
- **`/ui-automation`** — drives an Android emulator, an iOS simulator, or Chrome from the command line, reading the UI as structured text (accessibility tree / DOM) and acting on elements by name instead of screenshotting and guessing tap coordinates — roughly 5–10× cheaper per step, and it lands on the element instead of near it. Covers setup from nothing on each platform, ships `ui-android` and `ui-ios` helpers with a shared `tree` / `find` / `tap` / `type` / `key` / `shot` vocabulary, and documents the traps that silently break these loops (web content invisible to the tree, gestures that no-op, extension connections that fail three different ways). Screenshots stay one command away for when the question is genuinely visual.
- **`/slack-integration`** — the one-stop reference for designing and building anything on Slack's platform. Starts by picking the *shape* — incoming webhook, token script, no-code Workflow Builder flow, custom workflow step, slash command, Socket Mode app, HTTP app, AI agent, or MCP in either direction — since choosing wrong is where most of the wasted effort in this domain lives, then goes deep per axis in seven reference files: tokens and scopes, Block Kit and surfaces, events and delivery, deployment and local dev, app configuration and distribution, limits and pitfalls, and AI agents. Encodes the traps that cost real time (a bot only sees channels it was invited to, three-second acknowledgements, search needs a user token, Socket Mode can't be Marketplace-listed, `files.upload` is retired, history rate limits for unlisted apps, plan gates) plus a design-review checklist to run before writing code. Triggers on "Slack bot", "post to Slack", "Slack app", a Block Kit question, or a bare Slack API method name.
